...
def calculate_isotope_distances(l_mass_to_charges_raw: List) -> List[float]:
    """
    Calculate distances between isotopes based on mean m/z values.
    
    Args:
        l_mass_to_charges_raw: Nested list of m/z values [[isotope0_mz], [isotope1_mz], ...]
    
    Returns:
        List of distances between consecutive isotope mean m/z values.
        Empty list if fewer than 2 isotopes.
    """
    if not l_mass_to_charges_raw or len(l_mass_to_charges_raw) < 2:
        return []
    
    # Calculate mean m/z for each isotope
    isotope_means = []
    for isotope_mzs in l_mass_to_charges_raw:
        if isotope_mzs:
            mean_mz = sum(isotope_mzs) / len(isotope_mzs)
            isotope_means.append(mean_mz)
    
    # Calculate distances between consecutive isotopes
    if len(isotope_means) < 2:
        return []
    
    distances = []
    for i in range(len(isotope_means) - 1):
        distance = abs(isotope_means[i + 1] - isotope_means[i])
        distances.append(distance)
    
    return distances


def build_feature_list_by_mode(l_mass_to_charges_raw: List, l_intensities_raw: List, 
                                 l_retention_times_raw: List, feature_mode: str = "CoM") -> List[tuple]:
    """
    Build feature list (m/z, RT, intensity tuples) based on selected mode.
    
    Args:
        l_mass_to_charges_raw: Nested list of m/z values [[isotope1_mz], [isotope2_mz], ...]
        l_intensities_raw: Nested list of intensities [[isotope1_int], [isotope2_int], ...]
        l_retention_times_raw: Nested list of RTs [[isotope1_rt], [isotope2_rt], ...]
        feature_mode: "CoM" (all isotopes), "first_mean" (first isotope only), or "single_peak" (highest intensity peak)
    
    Returns:
        List of (m/z, RT in seconds, intensity) tuples
    """
    feature_ls = []
    
    if not l_mass_to_charges_raw:
        return feature_ls
    
    # Check if we have nested structure (multiple isotopes)
    if isinstance(l_mass_to_charges_raw[0], list):
        if feature_mode == "single_peak":
            # Use only the peak with highest intensity across all isotopes
            max_intensity = -1
            max_peak = None
            ...
            for mz_list, rt_list, int_list in zip(l_mass_to_charges_raw, l_retention_times_raw, l_intensities_raw):
                for mz, rt, intensity in zip(mz_list, rt_list, int_list):
                    ...
                    if intensity_val > max_intensity:
                        ...
                        max_peak = (float(mz), float(rt * 60), intensity_val)
            
            if max_peak:
                feature_ls.append(max_peak)
        
        elif feature_mode == "first_mean":
            # Use only the first isotope (index 0)
            if l_mass_to_charges_raw[0]:  # Ensure first isotope exists
                mz_list = l_mass_to_charges_raw[0]
                rt_list = l_retention_times_raw[0] if l_retention_times_raw else []
                int_list = l_intensities_raw[0] if l_intensities_raw else []
                
                for mz, rt, intensity in zip(mz_list, rt_list, int_list):
                    feature_ls.append((float(mz), float(rt * 60), float(intensity)))
        else:  # "CoM" or default - use all isotopes
            # Flatten all isotopes together
            for mz_list, rt_list, int_list in zip(l_mass_to_charges_raw, l_retention_times_raw, l_intensities_raw):
                for mz, rt, intensity in zip(mz_list, rt_list, int_list):
                    feature_ls.append((float(mz), float(rt * 60), float(intensity)))
    else:
        # Already flat list - no isotope structure
        if feature_mode == "single_peak":
...
def extract_feature_data(features_tsv: str, filename_base: str, round_up_to: int = 2, 
                         rt_start_trim: float = 0, rt_end_trim: float = 0, feature_mode: str = "CoM") -> List[Dict]:
    """
    ...
    Args:
        ...
        feature_mode: Method for calculating feature center: "CoM" (center of mass, all isotopes), "first_mean" (first isotope only), or "single_peak" (highest intensity peak)
    """
    ...
    for idx, row in features_data.iterrows():
        ...
        # Calculate isotope distances (mean m/z between consecutive isotopes)
        isotope_distances = calculate_isotope_distances(l_mass_to_charges_raw)
        
        pep_ident = parse_list_flexible(row['l_pep_ident'])
        raw_pep_ident = parse_list_flexible(row['l_raw_pep_ident']) if 'l_raw_pep_ident' in features_data.columns else []
        prot_ident = parse_list_flexible(row['l_prot_ident']) if 'l_prot_ident' in features_data.columns else []
        
        # Extract intensity and openms_fid from TSV
        intensity = float(row['intensity']) if 'intensity' in features_data.columns else 0.0
        openms_fid = row.get('id', row.get('openms_fid', str(idx))) if 'id' in features_data.columns or 'openms_fid' in features_data.columns else str(idx)
        ms2_scans = row.get('ms2_scans', row.get('l_ms2_scans', '')) if 'ms2_scans' in features_data.columns or 'l_ms2_scans' in features_data.columns else ''
        
        # Build feature_ls: list of (m/z, retention_time, intensity) tuples using selected mode
        feature_ls = build_feature_list_by_mode(l_mass_to_charges_raw, l_intensities_raw, 
                                                l_retention_times_raw, feature_mode)
        ...
        if mz_starts and mz_ends and rt_starts and rt_ends and feature_ls:
            ...
            if feature_ls:
                ...
                feature_data_list.append({
                    ...
                    'isotope_dist_mean': isotope_distances
                })
                box_count += 1
        else:
            skipped_count += 1
...
def main():
    ...
    parser.add_argument("--feature_mode", default="CoM", help="Feature center calculation mode: 'CoM' (all isotopes), 'first_mean' (first isotope), or 'single_peak' (strongest peak)")
