...
def generate_pairing_report(paired_json: str, output_summary: str, output_stats: str, edges_json: str = None, output_html: str = None, graph_composition_json: str = None, clusters_json: str = None, filtered_clusters_json: str = None, deleted_vertices_json: str = None, resolution_iterations_json: str = None, ident_lookup_json: str = None, metadata_json: str = None, parameters: Dict = None):
    ...
    with open(output_summary, 'w') as f:
        ...
        if cutoff_params:
            ...
            if parameters.get('mz_rt_weight_ratio') is not None:
                f.write(f"M/Z-RT Weight Ratio: {parameters.get('mz_rt_weight_ratio')}\n")
        ...
        if parameters.get('enable_clustering') or parameters.get('clustering_method'):
            ...
            if parameters.get('mega_complexity_threshold') is not None:
                f.write(f"Mega Complexity Threshold: {parameters.get('mega_complexity_threshold')}\n")
            f.write("\n")
    ...
    with open(output_stats, 'w') as f:
        ...
        if parameters.get('mz_rt_weight_ratio') is not None:
            f.write(f"mz_rt_weight_ratio,{parameters.get('mz_rt_weight_ratio')}\n")
...
def generate_html_report(
    ...
    if cutoff_params and any(v is not None for v in cutoff_params.values()):
        ...
        if parameters and parameters.get('mz_rt_weight_ratio') is not None:
            filter_items_html += f"<div>M/Z-RT Weight Ratio: <strong>{parameters.get('mz_rt_weight_ratio')}</strong></div>"
    ...
    """
        ...
        <div class="stats-section">
            ...
            <div class="stats-grid">
                ...
                <div class="stat-box">
                    <div class="stat-value">{parameters.get('mega_complexity_threshold', 'N/A') if parameters else 'N/A'}</div>
                    <div class="stat-label">Complexity Threshold</div>
                </div>
            </div>
        </div>
    """
    ...
    if parameters:
        ...
        tooltips = {
            ...
            'rt_alignment_outlier_threshold': 'Threshold for removing outliers in RT alignment: defines the size of the quartile of pep_ident pairs used to fit the curve (lower = more outliers removed)',
        ...
        alignment_params.append(('rt_alignment_outlier_threshold', 'Outlier Threshold'))
...
def main():
    ...
    parser.add_argument("--rt_alignment_outlier_threshold", type=float, default=0.1, help="Outlier threshold for RT alignment (as fraction of RT range)")
    ...
    parser.add_argument("--mega_complexity_threshold", type=float, default=None, help="Complexity threshold for heuristic vs exhaustive duplicate vertex filtering (from build_network_graph)")
    ...
    parameters = {
        ...
        'rt_alignment_outlier_threshold': args.rt_alignment_outlier_threshold,
