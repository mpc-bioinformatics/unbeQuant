...
params.mmf_compare_rt_alignment = true  // Enable RT alignment comparison and generate correction models
...
params.mmf_graph_output_graphml = false  // Export as GraphML format
...
params.mmf_graph_use_graphviz_clusters = false  // Group clusters into Graphviz subgraphs
...
params.mmf_clustering_resolution_parameter = 0.5//0.9756  // Resolution parameter for leiden clustering (default: 0.5, lower=more clusters, higher=fewer clusters)
...
process convert_clusters_to_consensusxml {
    ...
    ${workflow.projectDir}/bin/clusters_json_to_consensusxml.py \
        ...
        --mmf_graph_layout_engine ${params.mmf_graph_layout_engine} \
        --mmf_graph_layout_use_weights ${params.mmf_graph_layout_use_weights} \
    ...
    main:
        ...
        if (params.mmf_build_network_graph) {
            ...
            report_input = paired_output
                .combine(
                    graph_output.map { 
                        svg, graphml, gml, edgelist, analysis, composition, clusters, filtered, deleted, resolution_iterations, histogram ->
        ...
        emit:
            ...
            graph_svg = graph_output.map { it[0] }.ifEmpty(Channel.empty())
            graph_graphml = graph_output.map { it[1] }.ifEmpty(Channel.empty())
            graph_gml = graph_output.map { it[2] }.ifEmpty(Channel.empty())
            graph_edgelist = graph_output.map { it[3] }.ifEmpty(Channel.empty())
            graph_analysis = graph_output.map { it[4] }.ifEmpty(Channel.empty())
            graph_composition = graph_output.map { it[5] }.ifEmpty(Channel.empty())
            clusters_json = graph_output.map { it[6] }.ifEmpty(Channel.empty())
            filtered_clusters_json = graph_output.map { it[7] }.ifEmpty(Channel.empty())
            deleted_vertices_json = graph_output.map { it[8] }.ifEmpty(Channel.empty())
            graph_histogram = graph_output.map { it[9] }.ifEmpty(Channel.empty())
