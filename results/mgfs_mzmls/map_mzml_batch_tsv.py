...
log_scale = True  # If True, apply logarithmic scaling to intensities
scale_colors = False  # If True, scale colors based on min/max intensities, only for visualization
invert_colors = False  # If True, invert color scale (high intensity = dark)
...
generate_heatmap = False  # If True, generate heatmap images from mzML files (optional, not needed for feature pairing)
...
heatmap_directory = "/workspaces/unbeQuant/results/heatmaps/"
feature_analysis_directory = "/workspaces/unbeQuant/results/feature_analysis_plots/"
...
os.makedirs(feature_analysis_directory, exist_ok=True)
...
def intensity_to_rgb_lookup(intensity_int):
    ...
    if scale_colors and rgb_intensity_dict:
        return rgb_intensity_dict[intensity_int]
    else:
    ...
    if isinstance(s, str):
        try:
            ...
            if parsed and isinstance(parsed[0], list):
                # Nested list - convert inner lists to floats
                result = []
                ...
                # Single level list - convert each element to float
                result = []
                for x in parsed:
                    try:
                        result.append(float(x))
...
def create_heatmap_image(spec_total: np.ndarray, RTINSECONDS_arr: List[float], 
    ...
    for i in range(len(spec_total)):
        ...
        try:
            ...
            img_raw[index_sec][index_mz] = intensity_cur  # Store full precision intensity
...
def extract_feature_data(features_tsv: str, filename_base: str, img_raw: np.ndarray = None, mz_dict: Dict = None, 
                        rt_dict: Dict = None, mz_total_arr: List[float] = None, 
                        RTINSECONDS_arr: List[float] = None) -> List[Dict]:
    """
    Extract feature data and create overlays with boxes/CoM markers.
    Adapted to work with feature_ls structure containing lists of (m/z, retention_time, intensity).
    
    Parameters:
        features_tsv: Path to TSV file with feature data
        filename_base: Base filename for output
        img_raw: Optional raw intensity array from heatmap (for visualization overlays)
        mz_dict: Optional m/z to index mapping (for visualization overlays)
        rt_dict: Optional RT to index mapping (for visualization overlays)
        mz_total_arr: Optional m/z array (for visualization overlays)
        RTINSECONDS_arr: Optional RT array (for visualization overlays)
    """
    print(f"  Loading features from TSV...")
    features_data = pd.read_csv(features_tsv, sep='\t')
    print(f"    Loaded {len(features_data)} features")
    
    # Only compute img_height and img_width if heatmap data is available
    img_height = None
    img_width = None
    if RTINSECONDS_arr is not None and mz_total_arr is not None:
    ...
    for idx, row in features_data.iterrows():
        ...
        # Parse nested lists of actual coordinate data (arrays within the lists)
        # These come as nested structures from extract_hdf5_to_tsv.py
        ...
        # Handle nested list structure - each entry is a list of coordinates for one isotope
        if l_mass_to_charges_raw and isinstance(l_mass_to_charges_raw[0], list):
            # Nested list structure: zip and flatten coordinates with intensities across isotopes
            ...
            # Single level list structure: directly zip coordinates
        ...
        if mz_starts and mz_ends and rt_starts and rt_ends and feature_ls:
            ...
            if feature_ls:
                ...
                if feature_center_diagnostic:
                    fig = plt.figure(figsize=(14, 10))
                    
                    plt.subplot(2, 2, 1)
                    plt.scatter([x_center], [y_center], color='red', s=100, marker='x', label='Center of Gravity')
                    plt.scatter([x_center_geo], [y_center_geo], color='blue', marker='o', label='Geometric Center')
                    # Expand limits to ensure both centers are visible
                    mz_range = max(mz_coords) - min(mz_coords)
                    rt_range = max(rt_coords) - min(rt_coords)
                    margin_mz = max(mz_range * 0.1, 0.5)
                    margin_rt = max(rt_range * 0.1, 0.5)
                    plt.xlim(min(mz_coords) - margin_mz, max(mz_coords) + margin_mz)
                    plt.ylim(min(rt_coords) - margin_rt, max(rt_coords) + margin_rt)
                    plt.legend()
                    plt.title(f'Feature {idx} Centers (m/z, RT space)')
                    plt.xlabel('m/z')
                    plt.ylabel('Retention Time (s)')
                    
                    plt.subplot(2, 2, 2)
                    # Plot intensity distribution across feature coordinates with smaller dots
                    mz_vals = [f[0] for f in feature_ls]
                    rt_vals = [f[1] for f in feature_ls]
                    int_vals = [f[2] for f in feature_ls]
                    
                    scatter = plt.scatter(mz_vals, rt_vals, c=int_vals, s=5, cmap='hot', alpha=0.6)
                    plt.colorbar(scatter, label='Intensity')
                    plt.title(f'Feature {idx} Coordinate Distribution')
                    plt.xlabel('m/z')
                    plt.ylabel('Retention Time (s)')
                    
                    # Add table with center coordinates and intensity info
                    plt.subplot(2, 2, 3)
                    plt.axis('tight')
                    plt.axis('off')
                    ...
                    table_data = [
                        ['Property', 'Value'],
                        ['Geometric Center m/z', f'{x_center_geo:.4f}'],
                        ['Geometric Center RT (s)', f'{y_center_geo:.2f}'],
                        ['CoG Center m/z', f'{x_center:.4f}'],
                        ['CoG Center RT (s)', f'{y_center:.2f}'],
                        ['Intensity Sum', f'{intensity_sum:.2e}'],
                        ['Total Points', f'{len(feature_ls)}']
                    ]
                    ...
                    plt.title('Center & Intensity Information', pad=20)
                    
                    plt.tight_layout()
                    plt.savefig(os.path.join(feature_analysis_directory, f'feature_{idx}_analysis.png'), dpi=100, bbox_inches='tight')
                    plt.close()
                    bp() #Keep this, otherwise the programm creates way too many plots and crashes
...
def feature_pairing(all_feature_data_lists: List[List[Dict]]) -> List[Dict]:
    ...
    for feature_idx, ref_feature in enumerate(first_file_features):
        ...
        for file_idx in range(1, len(all_feature_data_lists)):
            ...
            # Calculate distances to all features in this file using center of mass coordinates
            distances = []
            for f in file_features:
                # Euclidean distance in (m/z, RT) space using intensity-weighted centers
...
def feature_pairing_optimized(all_feature_data_lists: List[List[Dict]]) -> List[Dict]:
    ...
    for file_idx, features in enumerate(all_feature_data_lists):
        # Extract coordinates for KD-tree using center of mass (x_center, y_center)
...
def process_single_file_pair(mzml_path: str, tsv_path: str, filename_base: str) -> Tuple[bool, List[Dict]]:
    ...
    try:
        ...
        # Initialize variables for optional heatmap generation
        img_raw = None
        mz_dict = None
        rt_dict = None
        mz_total_arr = None
        RTINSECONDS_arr = None
        
        # Step 1 & 2: Load mzML and create heatmap (optional)
        if generate_heatmap:
            print("  Generating heatmap from mzML...")
            spec_total, RTINSECONDS_arr, mz_total_arr, mz_dict, rt_dict = process_mzml_file(mzml_path)
            
            heatmap_output = os.path.join(heatmap_directory, f"{filename_base}_heatmap.png")
            ...
            print("  (Heatmap generation skipped - using feature data from TSV)")
        
        # Step 3: Extract feature data
        feature_data_list = extract_feature_data(tsv_path, filename_base, img_raw, mz_dict, rt_dict,
                                                  mz_total_arr, RTINSECONDS_arr)
...
def main():
    """
    Main entry point.
    Uses the global configuration variables for heatmap generation and feature pairing.
    """
    print("\n" + "="*70)
    print("Multi-File mzML to Feature Data Converter")
    print("="*70)
    print(f"Heatmap generation: {'ENABLED' if generate_heatmap else 'DISABLED'}")
    if optimize_pairing:
